...
def Generate_Custom_Conv(qconv2d_ref: QConv2d):
    ...
    custom_qconv2d = CustomConv2d(
        qconv2d_ref.in_channels,
        qconv2d_ref.out_channels,
        qconv2d_ref.kernel_size,
        qconv2d_ref.stride,
        qconv2d_ref.padding,
        groups=qconv2d_ref.groups,
    )
...
def Forward_Custom_Conv(Conv_block: Conv, x: torch.Tensor):
    ...
    res = dequantize_per_tensor(custom_conv2d(x), scale=qparams.scale, zero_point=qparams.zero_point)
    res = Conv_block.act(
        torch.quantize_per_tensor(res, scale=qparams.scale, zero_point=qparams.zero_point, dtype=torch.quint8)
    )
    return res
...
def Forward_Custom_v10Detect(head: Qv10Detect, x: list):
    ...
    res_one2one = [head.fl.cat((res_one2one_cv2[i], res_one2one_cv3[i]), 1) for i in range(head.nl)]

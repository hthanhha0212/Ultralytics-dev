...
from ultralytics.nn.modules.block import QC2f, SCDown, SPPF, QPSA, QC2fCIB, QBottleneck, QAttention, QRepVGGDW, QCIB
from ultralytics.nn.modules.head import Qv10Detect
from ultralytics.myimplm.utils_quant import compare_tensors, compare_list, customize_quantize, get_quant_params_qconv2d
...
def Compare_Conv_Block(conv_block: Conv, x_ref: torch.Tensor): 
    """
    This function use to generate an equivalent custom quantized conv2d and 
    compare the results
    """ 
...
def Compare_Functional_block(Func: QFunctional, opr1_ref: torch.Tensor, opr2_ref: torch.Tensor , list_ref: list = None, ops: str = ""):
...
def Compare_one2one_cv2_Block(one2one_cv2_block: ModuleList, x_ref: list, nl: int):
    res_ref = [
        one2one_cv2_block[i](x_ref[i]) for i in range (nl)
    ]
...
def Compare_one2one_cv3_Block(one2one_cv3_block: ModuleList, x_ref: list, nl: int):
    res_ref = [
        one2one_cv3_block[i](x_ref[i]) for i in range (nl)
    ]

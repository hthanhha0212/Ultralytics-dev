...
from ultralytics.myimplm.comparer import compare_list, compare_tensors
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once_do_compare(self, x, profile=False, visualize=False, embed=None):
        ...
        def log_comparison_row(block_name, mae=None, det_mae=None, det_y=None):
            if not self.log_cmpr:
                return
            LOGGER.info(f"{block_name:<{name_width}}{format_value(mae)}{format_value(det_mae)}{format_value(det_y)}")

        if self.log_cmpr:
            header = f"{'Block':<{name_width}}{'MAE':>{value_width}}{'DET_MAE':>{value_width}}{'DET_Y':>{value_width}}"
            LOGGER.info(header)
        ...
        for m in self.model:
            ...
            if not isinstance(m, (torch.nn.modules.upsampling.Upsample)):
                ...
                if not isinstance(m, Qv10Detect):
                    ...
                    if m.training:
                        ...
                        if avg_y != 0.0 or avg_mae != 0.0:
                            LOGGER.warning("Final results does not match with reference results ")
            else:
                x = m(x)
                x_cus = m(x_cus)
            ...
            if m.i in embed:
                embeddings.append(
                    torch.nn.functional.adaptive_avg_pool2d(x_cus, (1, 1)).squeeze(-1).squeeze(-1)
                )  # flatten
                if m.i == max_idx:
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        return x_cus
    ...
    def fuse_qat(self, verbose=True):
        """Fuse the `Conv2d()` and `BatchNorm2d()` layers of the model into a single layer for improved computation
        efficiency.

...
class QBottleneck(nn.Module):
    """Quantized Bottleneck, use to handle quantized tensors."""
...
class QC2f(nn.Module):
    """Quantized C2f, use quantized bottleneck version."""
...
class QAttention(nn.Module):
    """Quantized Attention module that performs its operations in quantized tensors.
...
class QPSA(nn.Module):
    """Quantizaed PSA module that used quantized Attention module.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.cv2(self.fl.cat((a, b), 1))


class QRepVGGDW(torch.nn.Module):
    """Quantized RepVGGDW, used to handle quantized tensors."""
    ...
    def __init__(self, ed: int) -> None:
        ...
        self.dim = ed
        # self.act = nn.SiLU()
        self.act = nn.ReLU()  # Use ReLU instead
        self.fl = FloatFunctional()
...
class QCIB(nn.Module):
    """Quantized Conditional Identity Block (CIB) module, use to handle quantized tensors, use quantized RepVGGDW
    instead.
...
class NPUFlatBottleneck(nn.Module):
    ...
    Identical computation to QBottleneck but uses standard Conv (groups=1) instead of DWConv, maximizing MAC array
    utilization on the NPU PE_CLUSTER. Also avoids FloatFunctional for the residual ADD — the compiler handles BP_ADD
    with proper scale parameters via BP_LOAD_PARAM.
    ...
    def __init__(
        self, c1: int, c2: int, shortcut: bool = True, g: int = 1, k: tuple = ((3, 3), (3, 3)), e: float = 1.0
    ):
...
class NPUFlatC2f(nn.Module):
    ...
    The standard QC2f uses FloatFunctional.cat(), which forces the NPU compiler to insert RESCALE atomics (each = 1 full
    identity 1×1 CONV
    dispatch) to harmonize quantization scales before concatenation.
    For n=1 bottleneck, this means 3 extra RESCALE dispatches adding ~300K+ cycles per C2f block.

    This block uses standard torch.cat() instead. During PTQ calibration, individual per-tensor observers on each chunk
    will independently determine their scales, and the compiler will use BP_CONCAT (virtual, zero cycles) with each
    tensor at its natural scale — no rescaling needed.
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        ...
        self.m = nn.ModuleList(
            NPUFlatBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=0.5) for _ in range(n)
        )
...
class NPUSimpleCIB(nn.Module):
    ...
    Each DWConv runs on single_cluster=1 mode on the NPU, halving the available MACs. The 7×7 DWConv alone costs 1.83M
    cycles.

    This simplified block replaces all 5 sub-layers with a standard inverted bottleneck (3 ops, all on both PE
    clusters):
    ...
    = 3–4 UOP_DISPATCH slots (vs. 7 in QCIB with 3 DWConv + 2 rescales). Both PE clusters active for all ops.
    """
...
class NPUC2fCIB(NPUFlatC2f):
    ...
    Replaces QC2fCIB (which uses DWConv 7×7 via QRepVGGDW + DWConv 3×3 wrappers) with a fully standard-convolution
    implementation for maximum NPU efficiency.
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(NPUSimpleCIB(self.c, self.c, shortcut, e=0.5) for _ in range(n))

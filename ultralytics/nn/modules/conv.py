...
class Pzconv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv2 = Conv(c1, c1, k=1, s=1)
        self.conv3 = nn.Conv2d(c1, c1, 5, 1, 2, groups=c1)
        self.conv4 = Conv(c1, c1, 1, 1)
        self.conv5 = nn.Conv2d(c1, c1, 7, 1, 3, groups=c1)
        self.proj = Conv(c1, c2, 1, 1) if c2 != c1 else nn.Identity()
        self.shortcut = Conv(c1, c2, 1, 1, act=False) if c2 != c1 else nn.Identity()
    ...
    def forward(self, x):
        ...
        x4 = self.conv4(x3)
        x5 = self.conv5(x4)
        x6 = self.proj(x5) + self.shortcut(x)
        return x6

...
from .block import DFL, QDFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN
from .conv import Conv, DWConv, QConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init
from torch.ao.quantization import QuantStub, DeQuantStub
from torch.nn.quantized import FloatFunctional

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "QDetect", "Qv10Detect"
...
class QDetect(nn.Module):
    """
    Quantized YOLO Detect head for object detection quantized models.
    This head use Quantized DFL module and be compatible with quantized input tensors
    ...
    def forward_end2end(self, x: list[torch.Tensor]) -> dict | tuple:
        ...
        one2one = [
            self.fl.cat((self.one2one_cv2[i](x_detach[i]), self.one2one_cv3[i](x_detach[i])), 1) for i in range(self.nl)
        ]

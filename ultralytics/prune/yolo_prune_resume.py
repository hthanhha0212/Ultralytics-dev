...
from ultralytics.nn.modules import QCIB, Conv, QAttention, QBottleneck, QC2f, QC2fCIB, Qv10Detect
from ultralytics.nn.tasks import load_checkpoint
from ultralytics.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, RANK, YAML
...
def load_resume_state(path: str | Path | None) -> dict | None:
    if not path:
        return None
...
def save_resume_state(path: str | Path | None, state: dict) -> None:
    if not path:
        return
    resume_path = Path(path)
...
def save_pruning_performance_graph(x, y1, y2, y3):
    """Draw performance change graph Parameters. ---------- x : List Parameter numbers of all pruning steps y1 : List
    mAPs after fine-tuning of all pruning steps y2 : List MACs of all pruning steps y3 : List mAPs after pruning
    (not fine-tuned) of all pruning steps.
...
def strip_optimizer_v2(f: str | Path = "best.pt", s: str = "") -> None:
...
def prune(args):
    ...
    if resume_state:
        if (
            resume_state.get("target_prune_rate") != args.target_prune_rate
            or resume_state.get("iterative_steps") != args.iterative_steps
        ):
            raise ValueError(
                "Resume state does not match the requested pruning configuration. "
    ...
    print(
        f"Before Pruning: MACs={base_macs_value / 1e9: .5f} G, #Params={base_nparams_value / 1e6: .5f} M, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="ultralytics/pretrained/weights/best.pt", help="Pretrained pruning target model file"
    )
    parser.add_argument(
        "--cfg",
        default="default.yaml",
        help="Pruning config file. This file should have same format with ultralytics/yolo/cfg/default.yaml",
    )
    parser.add_argument(
        "--dataset", default="data_config.yaml", help='Dataset config YAML passed to pruning_cfg["data"].'
    )
    parser.add_argument("--epochs", default=10, type=int, help="Fine-tuning epochs per pruning iteration step.")
